	...
	)
{
	...
	{
				WCHAR _ThreadExBuffer[ExBufferSize] = { 0 };
		NTSTATUS nsstatus = STATUS_UNSUCCESSFUL;
		if (myThreadEx)
		{
			nsstatus = RtlStringCbPrintfW(_ThreadExBuffer, ExBufferSize*sizeof(WCHAR), L"P%d-T%d:", finditem((int)PsGetCurrentThreadId()), (int)PsGetCurrentThreadId());
		}
		if (!NT_SUCCESS(nsstatus))
		{
			RtlZeroMemory(_ThreadExBuffer, ExBufferSize*sizeof(WCHAR));
		}
		//****
	...
	)
{
	...
	if (_myStatus && ((*_myHookedProcessID == (unsigned long)PsGetCurrentProcessId()) || (finditem((int)PsGetCurrentThreadId()) != -1 && (myThreadEx = 1))))
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"%sNtFsControlFile(handle[in]='%d')\r\n",_ThreadExBuffer,
			FileHandle);
		...
		{
			DbgPrint("Error in input buffer(NtFsControlFile) %d", _retStatus);
		}
	}
	return _retStatus;
}
