	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		WCHAR _buffer[MAX_LINE] = { 0 };
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtWriteVirtualMemory(handle[in]='%d',pvoid[in]='%p',pvoid[in]='%p')\r\n",
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtUnloadDriver()\r\n");
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtTerminateThread(handle[in]='%d')\r\n",
			ThreadHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtSystemDebugControl(pvoid[in]='%p',pvoid[out]='%p')\r\n",
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtSuspendThread(handle[in]='%d')\r\n",
			ThreadHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtSuspendProcess(handle[in]='%d')\r\n",
			ProcessHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtShutdownSystem()\r\n");
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtSetSystemInformation()\r\n");
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtSetSecurityObject(handle[in]='%d')\r\n",
			Handle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtSetContextThread(handle[in]='%d')\r\n",
			ThreadHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtSecureConnectPort(handle[out]='%d')\r\n",
			(long)*PortHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtRestoreKey(handle[in]='%d',handle[in]='%d')\r\n",
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtRequestWaitReplyPort(handle[in]='%d')\r\n",
			PortHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtRequestPort(handle[in]='%d')\r\n",
			PortHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtReplaceKey(handle[in]='%d')\r\n",
			Key);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtQueueApcThread(handle[in]='%d')\r\n",
			ThreadHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtOpenThread(handle[out]='%d',pclient_id[in]='%p')\r\n",
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtProtectVirtualMemory(handle[in]='%d',pvoid[in]='%p')\r\n",
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtOpenSection(handle[out]='%d')\r\n",
			(long)*SectionHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtMakeTemporaryObject(handle[in]='%d')\r\n",
			Handle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtFsControlFile(handle[in]='%d',handle[in]='%d',pvoid[in]='%p',pvoid[out]='%p')\r\n",
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		if (NT_SUCCESS(_retStatus))
		{
			if (TargetHandle != NULL)
				ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtDuplicateObject(handle[in]='%d',handle[in]='%d',handle[in]='%d',handle[out]='%d')\r\n",
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtDeviceIoControlFile(handle[in]='%d',handle[in]='%d',pvoid[in]='%p',pvoid[out]='%p')\r\n",
			FileHandle, Event, InputBuffer, OutputBuffer);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtCreateSection(handle[out]='%d',handle[in]='%d')\r\n",
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtCreateKey(handle[out]='%d')\r\n",
			(long)*KeyHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtConnectPort(handle[out]='%d')\r\n",
			(long)*PortHandle);
...
NTSTATUS HookNtAssignProcessToJobObject(__in HANDLE _job, __in HANDLE _process)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtAssignProcessToJobObject(handle[in]='%d',handle[in]='%d')\r\n",
			_job, _process);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtAlpcSendWaitReceivePort(handle[in]='%d')\r\n",
			PortHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtAlpcConnectPort(handle[out]='%d')\r\n",
			(long)*PortHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtAlpcCreatePort(handle[out]='%d')\r\n",
			(long)*PortHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtAllocateVirtualMemory(handle[in]='%d',pvoid[out]='%p')\r\n",
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtCreateThread(handle[out]='%d',handle[in]='%d',pclient_id[out]='%p'",
			(long)*ThreadHandle, ProcessHandle, ClientId);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtCreateThreadEx(handle[out]='%d',handle[in]='%d')\r\n",
			(long)*ThreadHandle, ProcessHandle);
...
)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtCreateProcessEx(handle[out]='%d',handle[in]='%d',handle[in]='%d',handle[in]='%d',handle[in]='%d')\r\n",
	...
	IN HANDLE ExceptionPort OPTIONAL)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtCreateProcess(handle[out]='%d',handle[in]='%d',handle[in]='%d',handle[in]='%d',handle[in]='%d')\r\n",
			(long)*ProcessHandle, ParentProcessHandle, SectionHandle, DebugPort, ExceptionPort);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtClose(handle[in]='%d')\r\n", _handle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtOpenKey(handle[out]='%d')\r\n", (long)*KeyHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtCreateFile(handle[out]='%d',pvoid[in]='%p')\r\n",
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtOpenFile(handle[out]='%d')\r\n",
			(long)*FileHandle);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtTerminateProcess(handle[in]='%d')\r\n",
			ProcessHandle);
	...
	else if (ProcessHandle == _myHookedProcessHandle)
	{
		WCHAR _buffer[MAX_LINE] = { 0 };
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtTerminateProcess called on this malware.\r\n");
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtLoadDriver()");
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		if (NT_SUCCESS(_retStatus))
			...
			L"NtOpenProcess(handle[out]='%d',pclient_id[in]='%p')\r\n", (long)*ProcessHandle, ClientId);
	...
	)
{
	...
	if (_myStatus && *_myHookedProcessID == (unsigned long)PsGetCurrentProcessId())
	{
		...
		NTSTATUS ntstatus = STATUS_UNSUCCESSFUL;
		if (NT_SUCCESS(_retStatus))
			ntstatus = RtlStringCbPrintfW(_buffer, sizeof(_buffer), L"NtDeleteValueKey(handle[in]='%d')\r\n", KeyHandle);
		if (NT_SUCCESS(ntstatus))
		{
			if (_logging(_buffer) == STATUS_SUCCESS)
